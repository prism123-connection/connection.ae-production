...
function OnBoardingPage() {
    ...
    const handleEnter = async () =>{
      setLoading(true)
      const result = await updateOnboardRole( imgLink );
      if (result.success) {
  ...
  return (
    <div className={` w-full flex bg-[#51C2FF]/10 relative justify-center min-h-screen overflow-hidden! ${loading ? 'items-center' : 'items-start'}`}>
              ...
              <span className={`text-5xl font-semibold bg-gradient-to-r from-[#51C2FF] via-[#A9EE86] to-[#FFA442] text-transparent bg-clip-text  decoration-[#51C2FF] leading-20 transition-opacity duration-2000 text-center ${loading ? 'opacity-0' : 'opacity-100' } `}>Welcome on board! {user} </span>
