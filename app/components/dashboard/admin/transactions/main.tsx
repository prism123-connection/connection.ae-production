            ...
            <span className="text-base  mb-8">Review all payment occured on connection platform</span>
              ...
              <ProductTable childClasses='grid-cols-[5%_30%_25%_10%_10%_10%_10%]' columns={orderColumns}>
                {submissions.map((order, index) => (
                    <TransactionRows className={`grid-cols-[5%_30%_25%_10%_10%_10%_10%] ${index === submissions.length-1 ? ' rounded-b-xl' : null}`} key={index} status={order.status} index={index} data={order} />
                ))}
            </ProductTable>
        </SectionHeader>
  )
